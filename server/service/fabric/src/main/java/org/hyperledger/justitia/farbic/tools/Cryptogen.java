...
public class Cryptogen {
    ...
    private Cryptogen(File cryptogen, File config) throws FabricToolsException {
        ...
        if (!config.exists()) {
            throw new FabricToolsException(String.format("Not found cryptogen's config file in directory %s.", config.getPath()));
        }
    ...
    private CallShell.Result callCryptogen(String command) throws FabricToolsException {
        ...
        try {
            ...
            throw new FabricToolsException("Cryptogen tool call failed.", e);
        }
    }
}
