...
import org.hyperledger.justitia.common.exception.FileUtilsException;
...
public class ZipFileUtils extends FileUtils {
    ...
    public File createWithSelf(String sourcePath, String zipPath) throws IOException {
        if (!zipPath.endsWith(".zip")) {
            throw  new RuntimeException(zipPath + "is not a zip file.");
        }
     ...
     * @return 加压后文件路径
     */
    public String unZip(String path, File file) throws IOException {
        String fileName = file.getName();
        String suffix = fileName.substring(fileName.lastIndexOf(".") + 1);
        if(!"zip".equals(suffix)) {
            throw new FileUtilsException("The suffix of the file is not zip.");
        }
