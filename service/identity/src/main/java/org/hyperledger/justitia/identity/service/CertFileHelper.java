...
public class CertFileHelper {
    ...
    /**
     * Convert private tlsKey in PKCS8 to PEM
     *
     * @param privateKey 私钥
     * @return pem编码的私钥
    ...
    public BigInteger getSerialNumber(String certPem) throws CertificateException {
        if (certPem == null) {
            throw new CertificateException("Certificate PEM is null");
        }
        CertificateFactory cf = CertificateFactory.getInstance("X.509");
...
//            certFileName = "tlsCert.pem";
//        }
//
//        String fileName = getSerialNumberString(tlsCert);
//        String tempDir = ExternalResources.getTemp(fileName);
//        //创建临时目录
//        FileUtils.makeDir(tempDir);
//        //保存证书和私钥到文件
//        FileUtils.writeStringToFile(tempDir, tlsCert, certFileName);
//        String privateKeyName = getFabricPrivateKeyName(tlsCert);
//        FileUtils.writeStringToFile(tempDir, tlsKey, privateKeyName);
//        //打包文件
