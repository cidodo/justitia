...
public class ConfigHelper {
    ...
    protected void notEmpty(Collection collection, String errorMsg) {
        if (null == collection || collection.isEmpty()) {
            throw new IllegalArgumentException(errorMsg);
        }
    }

    protected Map readCofnigFromYaml(File configFile) throws FileNotFoundException, FabricToolsConfigException {
        if (null == configFile) {
    ...
    protected File writeConfigToYaml(Map config, File output) throws IOException {
        if (null == output) {
